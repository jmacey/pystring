...
#define MAX_32BIT_INT 2147483647
    ...
    // template this so we can deal with return type
    template <typename S>
    std::vector<S> split(S str, std::string_view sep = "", int maxsplit = -1)
    {
        std::vector<S> result;
    ...
    // template this so we can deal with return type
    template <typename S>
    std::vector<S> rsplit(S str, std::string_view sep = "", int maxsplit = -1)
    {
        std::vector<S> result;
    ...
    namespace os
    {
        namespace path
        {
            ...
            std::string join_nt(const std::vector<std::string_view> &paths);
