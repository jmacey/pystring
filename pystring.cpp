...
#define ADJUST_INDICES(start, end, len) \
    ...
    namespace
    {
        ...
        void reverse_strings(std::vector<std::string_view> &result)
        {
            for (std::vector<std::string_view>::size_type i = 0; i < result.size() / 2; ++i)
            {
                std::swap(result[i], result[result.size() - 1 - i]);
            }
        }
        ...
        ///
        ///
        void split_whitespace(std::string_view str, std::vector<std::string_view> &result, int maxsplit)
        {
        ...
        ///
        ///
        void rsplit_whitespace(std::string_view str, std::vector<std::string_view> &result, int maxsplit)
        {
            ...
            for (i = j = len; i > 0;)
            {
                ...
                if (j > i)
                {
                    if (maxsplit-- <= 0)
                        break;
                    // should be able to constexpr but not allowing
            ...
            for (i = j = len; i > 0;)
            {
                ...
                if (j > i)
                {
                    if (maxsplit-- <= 0)
                        break;
                    // should be able to constexpr but not allowing
                    result.push_back(std::string(std::string(str.substr(i, j - i))));
    ...
    // String view vector version
    void split(std::string_view str, std::vector<std::string_view> &result, std::string_view sep, int maxsplit)
    {
        result.clear();
...
/*
    void rsplit(std::string_view str, std::vector<std::string_view> &result, std::string_view sep, int maxsplit)
    {
        if (maxsplit < 0)
        {
            split(str, result, sep, maxsplit);
...
#define BOTHSTRIP 2
    ...
    std::string join(std::string_view str, const std::vector<std::string_view> &seq)
    {
        ...
        if (seqlen == 0)
            return empty_string;
        if (seqlen == 1)
            return std::string(seq[0]);

        std::string result(seq[0]);

        for (i = 1; i < seqlen; ++i)
        {
            result += std::string(str) + std::string(seq[i]);
        }
    ...
    ///
    ///
    std::string title(const std::string_view str)
    {
        std::string s(str);
        std::string::size_type len = s.size(), i;
    ...
    namespace os
    {
        namespace path
        {
            ...
            void splitdrive_nt(std::string &drivespec, std::string &pathspec, std::string_view p)
            {
                if (p.size() >= 2 && p[1] == ':')
                {
                    std::string_view path(p); // In case drivespec == p
            ...
            bool isabs(const std::string_view path)
            {
#ifdef WINDOWS
            ...
            std::string join_nt(std::vector<std::string_view> &paths)
            {
                if (paths.empty())
                    return empty_string;
                if (paths.size() == 1)
                ...
                {
                    std::string b = std::string(paths[i]);
            ...
            {
                std::vector<std::string_view> paths(2);
                paths[0] = a;
                paths[1] = b;
            ...
            std::string join_posix(const std::vector<std::string_view> &paths)
            {
                if (paths.empty())
                    return empty_string;
                if (paths.size() == 1)
            ...
            std::string join(const std::vector<std::string_view> &paths)
            {
#ifdef WINDOWS
                return join_nt(paths);
#else
            ...
            std::string normpath_posix(const std::string &p)
            {
                ...
                if (initial_slashes && pystring::startswith(path, double_forward_slash) && !pystring::startswith(path, triple_forward_slash))
                    initial_slashes = 2;

                std::vector<std::string_view> comps, new_comps;
                pystring::split(path, comps, forward_slash);

                for (unsigned int i = 0; i < comps.size(); ++i)
                {
                    std::string_view comp = comps[i];
